...
/*
 * Funï¿½ï¿½es relacionadas a permissï¿½o
 */
...
function postRequired($url=null){
	if($_SERVER['REQUEST_METHOD'] != 'POST'){
		if($url != null){
			...
			throw new Exception('Este tipo de requisiï¿½ï¿½o nï¿½o ï¿½ permitido');
		}
	}
}
...
function isAuth(){
    if($_SERVER['X_REQUESTED_WITH'] == 'XMLHttpRequest'){
        Load::json(array('return' => true, 'message' => 'Vocï¿½ precisa estar autenticado para acessar esta ï¿½rea'));
    }
...
/**
 * Verifica se existe uma variï¿½vels next, 
 * se nï¿½o existir redireciona para url enviada por parametro 
 */
...
function getURLMPDF() {
	$absoluteURL = $GLOBALS['BASE_URL'] . $_SERVER['REQUEST_URI'];
	if(strstr($_SERVER['REQUEST_URI'], '?')) {
		return str_replace('?', '/pdf?', $absoluteURL);
	} else {
		return $absoluteURL . '/pdf';
	}
}

/*
 * Funï¿½ï¿½es relacionadas a formataï¿½ï¿½o
 */
...
function formatReadMore($text, $limit){
	...
	if($len > $limit){
		return '<a href="#" class="more" title="more"><img src="'.url('templates/default/img/read_more.png?20170801').'" width="15" /></a> '.substr($text, 0, $limit).' <span class="hide">'.substr($text, $limit, $len).'</span>';
	} else {
		return $text;
	}
}
...
function retiraAcentos($texto){
  $array1 = array('ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½', 'ï¿½'
...
function getMeses($i=null){
	...
	if(is_numeric($i)){
		$mes = array('1' => 'Janeiro', '2' => 'Fevereiro', '3' => 'Marï¿½o', '4' => 'Abril', '5' => 'Maio', '6' => 'Junho', 
					 '7' => 'Julho', '8' => 'Agosto', '9' => 'Setembro', '10' => 'Outubro', '11' => 'Novembro', 
					 '12' => 'Dezembro');
	} else {
		$mes = array('janeiro' => 'Janeiro', 'fevereiro' => 'Fevereiro', 'marco' => 'Marï¿½o', 'abril' => 'Abril', 'maio' => 'Maio', 'junho' => 'Junho',
...
function limpaDiretorio($dir) {
	...
	foreach($arquivos as $arquivo) {
        if(($arquivo != '.') && ($arquivo != '..') && $arquivo != '.gitignore') {
			unlink($dir.$arquivo);
		}
	}
